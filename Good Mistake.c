...
/*INIZIALIZZAZIONE VARIABILI PER I MOVIMENTI*/
bool neroSX;
bool lastBiancoSx;
bool lastBiancoDx;
int mode;
...
/*INIZIALIZZAZIONE VARIABILI PER I2C*/
...
/*FINE INIZIALIZZAZIONE VARIABILI PER I2C*/
...
/*VARIABILI PID*/
const float kP = 2;
const float kI = 0; //Non lo usiamo perchè non è un sistema lineare
const float kD = 1.4;
/*FINE VARIABILI PID*/
...
task update(){ //è un task parallelo, così mentre gira il main questo gli aggiorna le variabili dei colori e dei sensori vari
	while(1){
		sendI2CMsg(ARDUINO_PORT, &BufferTx[0], 10); //richiede all'arduino i dati e si aspetta 10 byte di risposta
		...
		readI2CReply(ARDUINO_PORT, &BufferRx[0], 7); //legge la risposta e la mette su buffer rx
		/*PACCHETTO NELLA MIA TESTA ERA COSI'*/
		bt = (bool)BufferRx[0]; // byte 0 pulsante on off (obbligatorio per regolamento)
		finecorsaDx = (bool)BufferRx[1]; //byte 1 fine corsa dx
		finecorsaSx = (bool)BufferRx[2]; //byte 1 fine corsa sx
		distanzaDavanti = BufferRx[3]; //sensore distanza davanti
		distanzaDietro = BufferRx[4]; //sensore distanza dietro
		distanzaSx = BufferRx[5]; //sensore distanza sinistra
		distanzaDx = BufferRx[6]; //sensore distanza destra
		...
		getColorRGB(sensorSx, redValueSx, greenValueSx, blueValueSx); /*Acquisizione dei valori dei canali rgb nelle variabili*/
		//IMPORTANTISSIMO+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
		//Il sensore di sx leggeva di meno quindi lo moltiplicavo, ovviamente non va fatto con un valore fisso ma è bene fare una cosa che lo tara in automatico a inizio pista
		...
		PID = p_value + i_value + d_value; //calcolo il pid
		if(blueValueDx>blueOnSilver && blueValueSx>blueOnSilver){ // se entrambi vedono l'argento
			mode=1; //modalità raccogli palline
		}
		if(error<-5 || error>5 || controlloNeroCC()) { //se l'errore è prossimo allo zero resetto il timer
			clearTimer(T1);
		}
		if(time1[T1]>Max_Timing){ //se è passato un tempo maggiore di quello prefissato per cui il robot è rimasto con un errore costante allora inizio procedura ricerca linea
			mode=2;
		}
		//if(controlloAlzatoCC()&&controlloAlzatoDx()&&controlloAlzatoSx()) {
		//	muori();
		//}
		if(bt==true){ //se il pulsante dell'arduino è premuto
			mode=-1; //modalità fermo
		} else {
			mode=0; //modalità linea
		}
		error_prec=error; //si prende l'errore precedente
	}
}
...
task main() {
	...
	distance=10;
	while (true){
		if(mode==-1){ //modalità tasto premuto quindi deve stare fermo
			muovi(0,0);
			delay(1);
		} else if(mode==0){//Segui linea
			if(distance<5) evitaOstacolo(); //se vede un ostacolo inizia la procedura che lo evita
